...
def injerk_data():
    records_list = get_all_records('regulation.log')
    ...
    for scheme in SCHEMES:
        ...
        for record in records_scheme:
            dict_qoe[scheme].append(record['qoe'])
            dict_PQ[scheme].append(record['PQ'])
            dict_rebuffer[scheme].append(record['rebuffer'])
            dict_smoothness[scheme].append(record['smoothness'])
            dict_latency[scheme].append(record['latency'])
            dict_bandwidth_before[scheme].append(record['bw_before'])
            dict_bandwidth_after[scheme].append(record['bw_after'])
            dict_timestamp[scheme].append(int(record['timestamp']) - timestamp_init)

        print('dict_qoe', dict_qoe[scheme])
