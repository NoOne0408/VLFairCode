...
class IP_UDP_Heuristic:
    ...
    def estimate(self, file_tuple):
        ...
        df = df[df['frame.time_epoch'] >= max_time - 4]
        ...
        # print("df_grp_udp",df_grp_udp)
        #这一段代码用于webrtc作为模型纠正，因为我们无法获取到webrtc相关的json文件，因此不使用
        ...
        df_merge = df_grp_udp.dropna()
        df_merge = df_merge.rename(columns={
                                       f'predicted_{self.metric}': f'{self.metric}_gt', self.metric:  f'{self.metric}_gt', 'ts': 'timestamp'})
        ...
        if self.metric == 'framesReceivedPerSecond':
            ...
            #     lambda x: round(x))
            df_merge[f'{self.metric}_gt'] = df_merge[f'{self.metric}_gt'].apply(
                lambda x: round(x))
        return df_merge
